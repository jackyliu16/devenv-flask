...
@code    :   https://github.com/jackyliu16/small-toys/blob/main/Convert_xlsx_Into_SQL_Insert_Query/generate_sql_from_csv.py
"""
import os
import sys
            ...
            with open(file_location, "a+") as FILE:
                for row in data[i]:
                    ...
                    for i in range(0, len(row) - 1):
                        logging.debug(f"{i}, {len(row)-1}")
                        if attr[i] == ColType.STRING:
                            sql += f"'{str(row[i])}',"
                        else:
                            sql += f"{row[i]},"
                    if attr[len(row) - 1] == ColType.STRING:
                        sql += f"'{str(row[i])}');\n"
                    else:
                        sql += f"{row[i]});\n"
...
def main(logLevel=logging.INFO):
    ...
    inp = ""
    print(
        "if you want to using a different xlsx file please input it's path as below: ",
        end="",
    )
    inp = input()
    if inp != "":
    ...
    inp = ""
    print(
        "if you want to using a different sql output file please input it's path as below: ",
        end="",
    )
    inp = input()
    if inp != "":
